...
- [](/zh-cn/)- 快速开始- 接入 Agent 工具- WorkBuddy/CodeBuddy
本页总览
...
#### 1. 安装 WorkBuddy/CodeBuddy[​](#1-安装-workbuddycodebuddy)
...
#### 4. 可选：验证 API Key[​](#4-可选验证-api-key)

Windows 用户可以在 PowerShell 中验证 API Key：


```
$env:DEEPSEEK_API_KEY=""curl https://api.deepseek.com/v1/chat/completions `  -H "Content-Type: application/json" `  -H "Authorization: Bearer $env:DEEPSEEK_API_KEY" `  -d &#x27;{"model":"deepseek-v4-flash","messages":[{"role":"user","content":"hi"}],"stream":false}&#x27;
```
